...
2. **Build Everything**
   Run `npm run build` in the root of the workspace. This ensures all packages, API, and web overlays are freshly compiled. 
   - After starting the build, use the `schedule` tool to wait exactly 60 seconds.
   - If the build is still running after 60s, wait in increments of 15 seconds. DO NOT run any intermediate commands (like `echo`) while waiting; just stop calling tools and let the timer wake you up.
...
3. **Run the Release Script**
   ...
   *Note: This script automatically bumps the desktop app version, creates a zip archive in the `releases/` folder, updates `version.json` (which the launcher reads to self-update), commits the changes, and pushes a new GitHub release using the `gh` CLI.*
   - After starting the script, use the `schedule` tool to wait exactly 240 seconds.
   - If it is not finished after 240s, wait in increments of 40 seconds. DO NOT run any intermediate commands while waiting.
