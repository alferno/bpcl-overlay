 ...
 *
 * Usage:
 *   npm run heroes:download-cdn-renders
 *   npm run heroes:download-cdn-renders -- --limit=5
 *   npm run heroes:download-cdn-renders -- --force
 *   npm run heroes:download-cdn-renders -- --slug=invoker,magnataur
 */
...
async function main() {
  ...
  console.log(`Downloading ${heroes.length} hero render poster PNG(s) from Steam CDN...`);
  ...
  await mapPool(
    ...
    async (hero) => {
      ...
      for (const candidate of candidates) {
        const urlPng = `${CDN_RENDER_BASE}/${candidate}.png`;
        const destPng = path.join(outDir, `${candidate}.png`);
        
        let pngExists = !force && fs.existsSync(destPng) && fs.statSync(destPng).size > MIN_BYTES_SKIP;
        ...
        try {
          await downloadFile(urlPng, destPng);
          
          savedAs = candidate;
          ok.push(candidate);
          const kbPng = (fs.statSync(destPng).size / 1024).toFixed(1);
          console.log(`  OK   ${candidate} (${kbPng} KB)`);
          break;
        } catch (e) {
          /* try next candidate */
        }
  ...
  if (missing.length) {
    console.warn("Some renders failed; manifest lists successful downloads only.");
